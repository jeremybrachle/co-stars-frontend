"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getDemoSnapshotBundle = getDemoSnapshotBundle;
exports.getDemoSourceLabel = getDemoSourceLabel;
const snapshotIndexes_1 = require("./snapshotIndexes");
const DEMO_MANIFEST = {
    version: "demo-1",
    sourceUpdatedAt: "2026-03-11T00:00:00.000Z",
    actorCount: 8,
    movieCount: 5,
    relationshipCount: 13,
    levelCount: 6,
    recommendedRefreshIntervalHours: 24 * 365,
    snapshotEndpoint: "/demo",
};
const DEMO_SNAPSHOT = {
    meta: {
        version: DEMO_MANIFEST.version,
        exportedAt: DEMO_MANIFEST.sourceUpdatedAt,
        actorCount: DEMO_MANIFEST.actorCount,
        movieCount: DEMO_MANIFEST.movieCount,
        relationshipCount: DEMO_MANIFEST.relationshipCount,
        levelCount: DEMO_MANIFEST.levelCount,
    },
    actors: [
        { id: 1, name: "George Clooney", popularity: 9.5 },
        { id: 2, name: "Brad Pitt", popularity: 9.1 },
        { id: 3, name: "Julia Roberts", popularity: 8.9 },
        { id: 4, name: "Matt Damon", popularity: 8.7 },
        { id: 5, name: "Sandra Bullock", popularity: 8.8 },
        { id: 6, name: "Keanu Reeves", popularity: 9.0 },
        { id: 7, name: "Carrie-Anne Moss", popularity: 7.9 },
        { id: 8, name: "Hugo Weaving", popularity: 7.8 },
    ],
    movies: [
        { id: 101, title: "Ocean's Eleven", releaseDate: "2001-12-07" },
        { id: 102, title: "Ocean's Twelve", releaseDate: "2004-12-10" },
        { id: 103, title: "The Mexican", releaseDate: "2001-03-02" },
        { id: 104, title: "Speed", releaseDate: "1994-06-10" },
        { id: 105, title: "The Matrix", releaseDate: "1999-03-31" },
    ],
    movieActors: [
        { movieId: 101, actorId: 1 },
        { movieId: 101, actorId: 2 },
        { movieId: 101, actorId: 4 },
        { movieId: 102, actorId: 1 },
        { movieId: 102, actorId: 3 },
        { movieId: 103, actorId: 2 },
        { movieId: 103, actorId: 3 },
        { movieId: 104, actorId: 5 },
        { movieId: 104, actorId: 6 },
        { movieId: 105, actorId: 6 },
        { movieId: 105, actorId: 7 },
        { movieId: 105, actorId: 8 },
    ],
    adjacency: {
        actorToMovies: {
            "1": [101, 102],
            "2": [101, 103],
            "3": [102, 103],
            "4": [101],
            "5": [104],
            "6": [104, 105],
            "7": [105],
            "8": [105],
        },
        movieToActors: {
            "101": [1, 2, 4],
            "102": [1, 3],
            "103": [2, 3],
            "104": [5, 6],
            "105": [6, 7, 8],
        },
    },
    levels: [
        { actorA: "George Clooney", actorB: "Brad Pitt", stars: 1 },
        { actorA: "Sandra Bullock", actorB: "Keanu Reeves", stars: 1 },
        { actorA: "George Clooney", actorB: "Julia Roberts", stars: 1 },
        { actorA: "Carrie-Anne Moss", actorB: "Keanu Reeves", stars: 1 },
        { actorA: "Matt Damon", actorB: "Julia Roberts", stars: 2 },
        { actorA: "Hugo Weaving", actorB: "Sandra Bullock", stars: 2 },
    ],
};
const DEMO_BUNDLE = {
    manifest: DEMO_MANIFEST,
    snapshot: DEMO_SNAPSHOT,
    indexes: (0, snapshotIndexes_1.buildSnapshotIndexes)(DEMO_SNAPSHOT),
    loadedFrom: "demo",
};
function getDemoSnapshotBundle() {
    return DEMO_BUNDLE;
}
function getDemoSourceLabel() {
    return "the built-in offline demo dataset";
}
